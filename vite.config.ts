...
// ESM alternative for __dirname in Vite 8
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? "Fortune Intern Network"
...
function figmaErrorOverlayReplay(): Plugin {
  return {
    ...
    configureServer(server) {
      ...
      server.ws.on('connection', () => {}) // Dummy placeholder to match your cut-off block
    },
  }
}

// Stubs for missing trailing references in your code block to guarantee successful compilation
function figmaReactRefreshBoundaryFallback(): Plugin { return { name: 'figma-react-refresh-fallback' } }
function figmaMakeKitPlugin(_options: { storiesGlob: string }): Plugin { return { name: 'figma-make-kit' } }
