...
function AdminChangePwModal({ onClose }: { onClose: () => void }) {
  ...
  const toggle = (i: number) =>
    setShows((s) => s.map((v, idx) => (idx === i ? !v : v)));
    ...
    <AdminModal title="Change Admin Password" onClose={onClose}>
      {done ? (
        <div className="text-center py-4">
          ...
          <button
            onClick={onClose}
            className="w-full bg-fin-navy hover:bg-fin-navy-light text-white font-bold py-3 rounded-xl text-sm transition-colors"
          >
            Done
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {["Current password", "New password", "Confirm new password"].map(
            (label, i) => (
              <div key={label}>
                <label className="block text-sm font-semibold text-fin-text mb-1.5">
                  {label}
                </label>
                <div className="relative">
                  <Lock
                    size={15}
                    className="absolute left-3.5 top-1/2 -translate-y-1/2 text-fin-muted"
                  />
                  <input
                    type={shows[i] ? "text" : "password"}
                    placeholder="••••••••"
                    className="w-full pl-10 pr-11 py-3 border border-fin-border rounded-xl text-sm focus:outline-none focus:border-fin-navy focus:ring-2 focus:ring-fin-navy/10 transition-all"
                  />
                  <button
                    type="button"
                    onClick={() => toggle(i)}
                    className="absolute right-3.5 top-1/2 -translate-y-1/2 text-fin-muted hover:text-fin-text transition-colors"
                  >
          ...
          <div className="flex gap-3 pt-1">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 border border-fin-border text-fin-navy font-semibold py-3 rounded-xl text-sm hover:bg-fin-ground transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex-1 bg-fin-navy hover:bg-fin-navy-light disabled:opacity-60 text-white font-bold py-3 rounded-xl text-sm transition-colors"
            >
...
const revenueData = [
  ...
  { month: "Aug", revenue: 14350 },
  { month: "Sep", revenue: 20600 },
];
...
export default function Admin({ onNavigate }: Props) {
  ...
  const stats = [
    {
      ...
      icon: Users,
      color: "bg-fin-blue-light text-fin-blue",
      trend: "+12% this month",
    },
    {
      label: "Employers",
      value: adminStats.totalEmployers.toString(),
      icon: Building2,
      color: "bg-fin-gold-light text-fin-gold-dark",
      trend: "+5 this month",
    },
    {
      label: "Internships",
    ...
    },
    {
      ...
      icon: FileText,
      color: "bg-fin-green-light text-fin-green",
      trend: "+125 this week",
    },
  ];
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <aside className="hidden md:flex flex-col w-56 bg-fin-navy flex-shrink-0">
        ...
        <div className="px-3 py-4 border-t border-white/10">
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold text-white/50 hover:text-white hover:bg-white/10 transition-all"
          >
        ...
        <div className="md:hidden flex overflow-x-auto gap-2 px-4 py-3 bg-white border-b border-fin-border">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => setSection(item.id as AdminSection)}
              className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${section === item.id ? "bg-fin-navy text-white" : "bg-fin-ground text-fin-muted"}`}
            >
              {item.label}
            </button>
          ))}
        ...
        <div className="p-4 md:p-6 animate-fade-in">
          ...
          {section === "dashboard" && (
            <div className="space-y-6">
              <div>
                <h2 className="text-2xl font-bold text-fin-navy">
                  Admin Dashboard
                </h2>
                <p className="text-fin-muted text-sm mt-1">
                  Platform overview · September 9, 2026
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {stats.map((s) => (
                  <div
                    key={s.label}
                    className="bg-white rounded-2xl p-5 border border-fin-border shadow-sm"
                  >
                    <div
                    ...
                    <div className="text-2xl font-bold text-fin-navy">
                      {s.value}
                    </div>
                    <div className="text-xs text-fin-muted font-medium mt-0.5">
                      {s.label}
                    </div>
                    <div className="text-[10px] text-fin-green font-semibold mt-1">
                      {s.trend}
                    </div>
                  </div>
                ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="bg-white rounded-2xl border border-fin-border p-5">
                  <h3 className="font-bold text-fin-navy text-sm mb-1">
                    Application Volume
                  </h3>
                  <p className="text-fin-muted text-xs mb-4">
                ...
                <div className="bg-white rounded-2xl border border-fin-border p-5">
                  ...
                  <p className="text-fin-muted text-xs mb-4">
                    Monthly application fee revenue
                  </p>
                  <ResponsiveContainer width="100%" height={140}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-fin-navy rounded-2xl p-5 text-center">
                  ...
                  <div className="text-5xl font-bold text-fin-gold">
                ...
                <div className="bg-white rounded-2xl border border-fin-border p-5 text-center">
                  <p className="text-fin-muted text-xs font-medium uppercase tracking-wide mb-2">
                    Revenue This Month
                  </p>
                  <div className="text-3xl font-bold text-fin-navy">
                    GHS {adminStats.revenueThisMonth.toLocaleString()}
                  </div>
                  <p className="text-fin-muted text-xs mt-2">
                    {adminStats.paymentsThisMonth} payments processed
                  </p>
                </div>
                <div className="bg-white rounded-2xl border border-fin-border p-5 text-center">
                  <p className="text-fin-muted text-xs font-medium uppercase tracking-wide mb-2">
                  ...
                  <div className="text-3xl font-bold text-fin-navy">
                    {adminStats.activePrograms}
                  </div>
                  <p className="text-fin-muted text-xs mt-2">
                    FIN Academy courses running
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-5">
              <div className="flex items-center justify-between">
                ...
                <div className="relative">
                  <Search
                    size={14}
                    className="absolute left-3 top-1/2 -translate-y-1/2 text-fin-muted"
                  />
                  <input
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    placeholder="Search applications..."
                    className="pl-8 pr-4 py-2 border border-fin-border rounded-xl text-sm focus:outline-none focus:border-fin-blue bg-white w-56"
                  />
                </div>
              </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-fin-border bg-fin-ground">
                        <th className="text-left px-4 py-3 text-xs font-bold text-fin-muted">
                          Reference
                        </th>
                        <th className="text-left px-4 py-3 text-xs font-bold text-fin-muted">
                          Student
                        </th>
                        <th className="text-left px-4 py-3 text-xs font-bold text-fin-muted hidden md:table-cell">
                          Internship
                        </th>
                        <th className="text-left px-4 py-3 text-xs font-bold text-fin-muted hidden lg:table-cell">
                          Company
                        </th>
                        <th className="text-left px-4 py-3 text-xs font-bold text-fin-muted">
                          Status
                    ...
                    <tbody>
                      {adminApplications
                        ...
                        .map((app) => (
                          <tr
                            key={app.id}
                            className="border-b border-fin-border last:border-0 hover:bg-fin-ground transition-colors"
                          >
                            <td className="px-4 py-3 font-mono text-xs text-fin-muted">
                              {app.id}
                            </td>
                            <td className="px-4 py-3">
                              <p className="font-semibold text-fin-navy text-xs">
                                {app.student}
                              </p>
                              <p className="text-fin-muted text-[10px]">
                                {app.university}
                              </p>
                            </td>
                            <td className="px-4 py-3 hidden md:table-cell text-xs text-fin-text">
                              {app.internship}
                            </td>
                            <td className="px-4 py-3 hidden lg:table-cell text-xs text-fin-text">
                              {app.company}
                            </td>
                            <td className="px-4 py-3">
                              <span
                                className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${statusColors[app.status]}`}
                              >
                                {app.status}
                              </span>
                            </td>
                            <td className="px-4 py-3 hidden md:table-cell">
                              <span
                                className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${statusColors[app.payment]}`}
                              >
                                {app.payment}
                              </span>
                            </td>
                            <td className="px-4 py-3 hidden lg:table-cell text-xs text-fin-muted">
                              {app.submitted}
                            </td>
                          </tr>
                        ))}
          ...
          {section === "students" && (
            <div className="space-y-5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-bold text-fin-navy bg-white border border-fin-border px-3 py-1.5 rounded-xl">
                  {adminStats.totalStudents.toLocaleString()} total
                </span>
              </div>
              <div className="bg-white rounded-2xl border border-fin-border overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {adminApplications.map((app, i) => (
                        <tr
                          key={i}
                          className="border-b border-fin-border last:border-0 hover:bg-fin-ground"
                        >
                          <td className="px-4 py-3 font-semibold text-fin-navy text-sm">
                            {app.student}
                          </td>
                          <td className="px-4 py-3 hidden md:table-cell text-xs text-fin-text">
                            {app.university}
                          ...
                          <td className="px-4 py-3 text-xs font-bold text-fin-blue">
                            2
                          </td>
                        </tr>
                      ))}
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {employers.map((emp) => (
                        ...
                        >
                          <td className="px-4 py-3">
                            <p className="font-semibold text-fin-navy">
                              {emp.name}
                            </p>
                          ...
                          <td className="px-4 py-3">
                            <span
                              className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${statusColors[emp.status]}`}
                            >
                              {emp.status}
                            </span>
                          </td>
          ...
          {section === "payments" && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-fin-muted text-sm mt-1">
                  GHS {adminStats.revenueThisMonth.toLocaleString()} revenue
                  this month · {adminStats.paymentsThisMonth} transactions
                </p>
              </div>
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  },
                  {
                    label: "Failed",
                    value: "0",
                    color: "bg-fin-red-light text-fin-red",
                  },
                ].map((s) => (
                  <div
                    key={s.label}
                    className="bg-white rounded-2xl border border-fin-border p-4 text-center"
                  >
                    <div
                      className={`text-2xl font-bold mb-1 ${s.color.split(" ")[1]}`}
                    >
                      {s.value}
                    </div>
                    <div
                      className={`text-xs font-bold px-2 py-0.5 rounded-full inline-block ${s.color}`}
                    >
                      {s.label}
                    </div>
                  </div>
                    ...
                    <tbody>
                      {adminPayments.map((p) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 font-bold text-fin-navy text-sm">
                            {p.amount}
                          </td>
                          <td className="px-4 py-3">
                            <span
                              className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${statusColors[p.status]}`}
                            >
                              {p.status}
                            </span>
                          </td>
                          <td className="px-4 py-3 hidden md:table-cell text-xs text-fin-muted">
                            {p.date}
                          </td>
                        </tr>
                      ))}
          ...
          {section === "automation" && (
            <div className="space-y-5">
              ...
              <p className="text-fin-muted text-sm">
                Monitor and manage FIN's automated workflows.
              </p>
              ...
              {[
                ...
                <div
                  key={i}
                  className={`bg-white rounded-2xl border ${workflow.color} p-5`}
                >
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-9 h-9 rounded-xl ${workflow.iconColor} flex items-center justify-center`}
                      >
                        <Zap size={16} />
                      </div>
                      <h3 className="font-bold text-fin-navy">
                        {workflow.title}
                      </h3>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className="w-2 h-2 rounded-full bg-fin-green animate-pulse-dot" />
                      <span className="text-xs font-semibold text-fin-green">
                        Active
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2 mb-3">
                    {workflow.steps.map((step, j) => (
                      <div key={j} className="flex items-center gap-1.5">
                        <span className="text-xs font-medium bg-white border border-fin-border text-fin-navy px-2.5 py-1 rounded-full">
                          {step}
                        </span>
                        {j < workflow.steps.length - 1 && (
                          <span className="text-fin-muted text-xs">→</span>
                        )}
                      </div>
                    ))}
                  </div>
          ...
          {section === "letters" && (
            <div className="space-y-6">
              ...
              <div>
                <h3 className="font-bold text-fin-navy text-base mb-3">
                  Generated Internship Letter Preview
                </h3>
                <div className="bg-white rounded-2xl border border-fin-border overflow-hidden">
                  ...
                  >
                    <p className="text-fin-muted text-sm mb-6">
                      September 9, 2026
                    </p>
                    <h3 className="font-bold text-fin-navy text-lg mb-1">
                      Letter of Internship Placement
                    </h3>
                    <p className="text-xs text-fin-muted mb-6 font-sans">
                      Reference: FIN-LETTER-2026-00847
                    </p>
                    <p className="text-fin-text text-sm leading-relaxed mb-4">
                      Dear <strong>Amara Johnson</strong>,
                    </p>
                    <p className="text-fin-text text-sm leading-relaxed mb-4">
                      We are pleased to inform you that you have been
                      successfully placed for an internship with{" "}
                      <strong>MTN Ghana</strong> through the Fortune Intern
                      Network (FIN) placement program.
                    </p>
                    <div className="bg-fin-ground rounded-xl p-4 mb-4 space-y-2">
                      {[
                        ...
                        >
                          ...
                          <span className="font-medium text-fin-navy">
                            {value}
                          </span>
                        </div>
                      ))}
                    ...
                    <p className="text-fin-text text-sm leading-relaxed mb-8">
                      This letter serves as official confirmation of your
                      internship placement. Please present this letter to your
                      university's internship coordinator. Congratulations and
                      best of luck in your placement!
                    </p>
                    <div className="border-t border-fin-border pt-4">
                      <p className="text-fin-navy font-bold text-sm">
                        FIN Placement Team
                      </p>
                      <p className="text-fin-muted text-xs">
                        Fortune Intern Network ·
                        placements@fortuneinternnetwork.com
                      </p>
                    </div>
                  </div>
                  ...
                  {[
                    {
                      ...
                      company: "MTN Ghana",
                      status: "sent",
                      time: "Sep 4, 2026 · 08:00 AM",
                    },
                    {
                      student: "Amara Johnson",
                      company: "Ecobank Ghana",
                      status: "sent",
                      time: "Sep 7, 2026 · 09:01 AM",
                    },
                    {
                      student: "Kwame Asante",
                      company: "Hubtel",
                      status: "sent",
                      time: "Sep 2, 2026 · 10:30 AM",
                    },
                    {
                      student: "Ama Owusu",
                      company: "Vodafone Ghana",
                      status: "sent",
                      time: "Aug 31, 2026 · 11:00 AM",
                    },
                  ].map((email, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between px-5 py-3.5 border-b border-fin-border last:border-0"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-fin-navy">
                            {email.student} → {email.company}
                          </p>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold bg-fin-green-light text-fin-green px-2 py-0.5 rounded-full">
                          Sent ✓
                        </span>
                        <span className="text-xs text-fin-subtle hidden md:block">
                          {email.time}
                        </span>
                      </div>
                    </div>
          ...
          {section === "analytics" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="bg-white rounded-2xl border border-fin-border p-5">
                  <h3 className="font-bold text-fin-navy text-sm mb-4">
            ...
            <div className="space-y-5 max-w-xl">
              ...
              <div className="bg-white rounded-2xl border border-fin-border overflow-hidden">
                <div className="bg-fin-navy px-5 py-5 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-white/60 text-xs">
                      admin@fortuneinternnetwork.com
                    </p>
                ...
                <div className="divide-y divide-fin-border">
                  {[
                    ...
                    {
                      label: "Last Login",
                      value: "September 9, 2026 · 09:14 AM",
                    },
                  ].map((item) => (
                    <div
                      key={item.label}
                    ...
                    >
                      <div>
                        <p className="text-[10px] text-fin-muted">
                          {item.label}
                        </p>
                        <p className="text-sm font-medium text-fin-navy">
                          {item.value}
              ...
              <div className="bg-white rounded-2xl border border-fin-border overflow-hidden">
                <div className="px-5 py-4 border-b border-fin-border">
                  <h3 className="font-bold text-fin-navy text-sm flex items-center gap-2">
                    <Lock size={14} /> Security
                  </h3>
                </div>
                <button
                  onClick={() => setShowChangePw(true)}
                  className="w-full flex items-center justify-between px-5 py-4 hover:bg-fin-ground transition-colors text-left"
                >
                  <div className="flex items-center gap-3">
                ...
                <div className="flex items-center justify-between px-5 py-4 border-t border-fin-border">
                  ...
                  <span className="text-[10px] font-bold bg-fin-gold-light text-fin-gold-dark px-2 py-0.5 rounded-full">
                    Recommended
                  </span>
                </div>
              </div>
              ...
              <div className="bg-fin-blue-light border border-fin-blue/20 rounded-xl px-4 py-3 flex items-center gap-3">
                ...
                <p className="text-xs text-fin-blue leading-relaxed">
                  Administrator accounts cannot be self-deleted. Contact your
                  system owner to remove admin access.
                </p>
              </div>
              ...
              <button
                onClick={() => onNavigate("landing")}
                className="w-full flex items-center justify-center gap-2 bg-white border border-fin-border text-fin-navy font-bold py-3.5 rounded-xl hover:bg-fin-ground transition-colors"
              >
      ...
      {/* Modal */}
      {showChangePw && (
        <AdminChangePwModal onClose={() => setShowChangePw(false)} />
      )}
    </div>
  );
}
